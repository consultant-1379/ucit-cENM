...
DEFAULT_SUBCHART="eric-enm-credm-controller"
...
usage_detailed ()
{
  ...
  usage
	...
	echo "  -c chart_name"
	echo "     Specify from which subchart get the version (Default $DEFAULT_SUBCHART)"
	echo "  -q"
...
main ()
{
  ...
  chart_version=$(wget -O - "$stateless_url" | tar zxOf - "eric-enm-stateless-integration/charts/$PRJ/Chart.yaml" | grep '^version:'| sed -e 's/version: //' -e 's/  *//g')
  print_out "-n" "Chart: $PRJ version:"
  echo "$chart_version"
}
